...
Note that the division into parts is meant to guide you in the order of implementing things. The idea is that you should add each part in sequence and then fully debug it. At the end, however, you need to submit a single program that contains the functionality of all the parts.
...
>M<erge ELF Files
>Q<uit
Note that the menu should use the same technique as in lab 1, i.e. an array of structures of available options. Toggle Debug Mode is as in Lab 4. Quit should unmap and close any mapped or open files, and "exit normally". Examine ELF Files queries the user for an ELF file name(s) to be used and examined henceforth. For now, options S, N, R, C, and M should call stub functions that print "not implemented yet". All file input should be read using the mmap system call. You are NOT ALLOWED to use read, or fread.
...
Bytes 1,2,3 of the magic number (in ASCII). Henceforth, you should check that the magic number (including the unprintable byte 0 of the header) is consistent with an ELF file, and refuse to continue if it is not.
...
Hints:
Symbols are listed in the designated sections. The section in which a symbol is defined (if it is defined) is the index of the symbol (the st_shndx field), which is an index into the section header table, referring to the section header of the appropriate section, and from there the section name can be retrieved as above. Symbol name is an attribute of the symbol structure, but recall again that the actual name string is stored in a string table, a separate section(.strtab).

Part 2b - Relocations
Extend your myELF program from part 2a to support an option that displays information on all the relocation table entries in a 32-bit ELF file.
The new Print Relocations option, for each open ELF file, should visit all the relocation information in that ELF file (if none, print "No relocations" and return). For each relocation, print its index number, the location (offset) it refers to, the name of the related symbol, tand its type. Note that the last 2 items are encoded in the r_info field and need to be extracted. Format should be:


File <ELF-file-name> relocations:
[index] location related_symbol_name type
[index] location related_symbol_name type
...
Verify your output is correct by comparing it to the output of readelf. In debug mode you should first print the size of each symbol table, the number of symbols therein, and any other useful information.

Part 3 - Linker pass 1
In this part you shall write a limited version of a linker pass 1 (as mentioned in lecture 5). It will merge 2 ELF files (the ones opened by Examine ELF File) into one relocateable object file. In this part, you should be using as input the files from the the "ELF object file examples" folder, where "F1a.o", F1b.o", or "F1c.o" are intended for use as the first file, and "F2a.o" or "F2b.o" are intended for use as the second file. Merging files should result in a single relocatable object file, mimicking the one generated by pass I of the linker. For example, invoking pass I of the linker: "ld -r -m elf_i386 F1a.o F2a.o -o F12a.ro" generates the relocatable object file "F12a.ro". Your program should generate the same type of output ELF file given these same files. The linker can be used on (only) the resulting file to create a working executable file: "ld -m elf_i386 F12a.ro -o F12a" generates the "F12a" executable file that can actually be run.
...
Search for sym in SYMTAB2, the symbol table of the other ELF file, and then:
...
Note that doing the loop once for each file as stated above may find and print some errors twice, e.g. if a symbol is defined in both files. Still, this is needed to make sure of finding all errors, such as a symbol that is UNDEFINED in the second file but does not appear in the first file at all. As a bonus, you may fix this (e.g. when looping over symbols in the second file, check only symbols which did not appear in the first file), but this is not a requirement in this assignment.

Part 3.2: Partial Merge of ELF Files
...
With all the above symplifying assumptions, the following method can be used for merging.
...
The symbol table of the first ELF file can be used as that of the merged file in this part.
...
The basic requirement here is that "readelf" on "out.ro" will show that all sections have been merged as stated above, except for the symbol table which is just a copy of the original symbol table. Note that for this task, the output should be validated mainly using tools such as readelf, and should not necessarily be expected to be byte-identical to the output of ld -r. Additionally, direct section concatenation is sufficient herem no need to observe things like section alignment, if any.

Part 3.3: Finalizing the Merge of ELF Files
(This part is a bonus, not required to get a full grade but is a significant achievement for good hackers.)

Before writing the symbol table to "out.ro", reslove the symbols. That is, copy over symbol values and definition of symbols from the second ELF file, for every symbol that is UNDEFINED in the first ELF file,

The basic requirement above was that "readelf" on "out.ro" will show that all sections have been merged as stated above. Now the new symbol table has to have all the symbols resolved, when operating on pairs of ELF files that obey the restrictions stated above. For the bonus, running ld (the linker pass II) on "out.ro" should work correctly in these cases, and generate an executable file that runs correctly!

Deliverables:
You need to submit "myELF.c" and a makefile which compiles and links your code. It is recommended to ensure that the makefile compiles the program with -m32, since the lab is based on 32-bit ELF structures.
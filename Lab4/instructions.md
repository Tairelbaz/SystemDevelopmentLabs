...
The rest of the functions will be written in the next tasks. The menu should be extensible, you will change and extend the functionality it in each sub-task of task 1. It should be printed using a loop iterating over the menu array, and be {NULL, NULL, NULL} terminated, as in lab 1.
...
Suppose that the user has already set the file name to "abc". If users choose L on the menu, they are prompted for location and length. It should look as follows (exact output format is not critical):
L
...
This option replaces length units (each of size determined by the current unit size) at target-location of file_name with bytes from the hexeditplus memory starting at virtual address source-address. Note that the filename is the last file name set by option F "Set File Name".

When the user chooses option S, the program should query the user for:
...
When the user chooses option y, the program should query the user for:
location (memory buffer location, in hexadecimal)
val (new value, in hexadecimal)

For example, if unit size was set to 4, choosing option "Memory Modify" using location 0x40, val 0x804808a, will overwrite the 4 bytes starting at location 0x40, with the new value 804808a. It should look as follows:
y
Please enter <location> <val>
40 804808a

As in the previous task, you should check that the location chosen to be modified, given the current unit size, is valid, and act accordingly.
...
Run the file.
Which "function" should precede main( ) in execution? (Hint: see assembly code in Lab 3).
...
You should do it using options L, y, and S in hexeditplus.

...
Display a prompt - the current working directory (see man getcwd). The path name is not expected to exceed PATH_MAX (defined in linux/limits.h, so you'll need to include it).
...
You must place the full path of an executable file in-order to run properly. For instance: "ls" will not work, whereas "/bin/ls" runs properly. (Why?)
...
nuke <process id> - Force termination of an entire process group (SIGKILL to the process group headed by ID). You should compile Printers.c and run the resulting program in the background to get 2 processes as a new process group.

In all cases, use the kill( ) system call wrapper, see man 2 kill, to send the relevant signal to the given process id or process group.
...
Your task: Implement a simple program called mypipe (a program separate from your shell), which creates a child process, sends a message (given as argv[1] to mypipe) to the child process, closes the pipe write end, and terminates. The child prints the incoming message and terminates. Use the pipe system call (see man) to create the pipe.
